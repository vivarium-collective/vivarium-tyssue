...
from tyssue.topology.sheet_topology import cell_division, drop_face
...
def update_stem_cells(eptm):
    ...
    eptm.face_df.loc[(eptm.face_df["boundary"] == 1) & (eptm.face_df["z"] > 0), "dying_cell"] = 1
...
def apoptosis_cylinder(sheet, manager, death_rate, dt, radius, geom):
    ...
    manager.append(apoptosis_cylinder, death_rate=death_rate, dt=dt, radius=radius)
    fix_points_cylinder(sheet, radius=radius)
